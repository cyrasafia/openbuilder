...
| E | `conversationFor` 既有 conv + `refreshListAndWorkingSse` 活跃 conv 的 reconcile/load/reload（`server_store.dart:258/260/262`、`:521/523`） | reconcile 完成后链式 `_backfillPreview` 回填 `_lastMessage`，覆盖 SSE 错过事件（后台/idle/其他端发消息）后的列表预览缺口（见 §6.6，LPS-14） |
...
```dart
// conversationFor 既有 conv 路径（server_store.dart:257-263）
if (force) {
...
```dart
// refreshListAndWorkingSse 活跃 conv 路径（server_store.dart:518-524）
...
- **链式（`.then`）而非并发 `unawaited`**：确保回填在 reconcile **完成**后读 `conv.lastMessagePreview()`，读到合并后的最新消息；并发则可能读到 reconcile 前的旧/空 `_messages`。`conversationFor` 新建 conv 路径（`:269-273`）已调 `_backfillPreview`（保留 MA-3 行为），但其 `unawaited(load); unawaited(_backfillPreview)` 并发竞态属既有行为，本路径不触碰。
- **reconcile 失败不回填**：`reconcile()` 失败时 `.then` 不执行（Future 走 reject）→ 保旧预览，与「reconcile 失败保留 SSE 累积」（`conversation_store.dart:337-350`）语义一致。
- **仅覆盖活跃 conv**：`refreshListAndWorkingSse` 对非活跃 conv 仅 `markStale()`（`:526-532`，延迟到下次打开 reconcile），不在刷新当下拉消息——非活跃会话的 per-session 回填见 §10.1（LPS-15 follow-up）。
...
- **（LPS-15）非活跃会话 per-session 回填**：E 路径仅覆盖活跃 conv（`refreshListAndWorkingSse` 活跃 conv 路径）；非活跃 conv 仅 `markStale()` 延迟到下次打开，刷新当下不拉消息。session REST 无 preview 字段（`SessionModel` `models.dart:95-148`），要回填须按需 `client.messages(sid)` 循环——成本与收益需权衡（多会话场景的请求数）。属 follow-up 单独评估，不阻塞 E 路径。
...
12. （E 路径，三次修订 LPS-14）单测：向 conv 注入一条"后台前旧预览"消息 → `_lastMessage[sid]` 设为旧值；再让 reconcile 注入一条新消息（其他端发的）→ 断言 reconcile 完成后 `_lastMessage[sid]` 被 `_backfillPreview` 更新为新消息预览（验证 `.then` 链式而非并发）；reconcile 失败路径断言 `_lastMessage[sid]` 保留旧值。
...
| 编号 | 修正内容 | 复审 |
|------|----------|------|
| LPS-14 | 新增 §6.6「E — REST reconcile 完成后回填 `_lastMessage`」：场景/根因（5 个写入点无 reconcile 桥接）/修复（`conversationFor:257-263` 既有 conv + `refreshListAndWorkingSse:518-524` 活跃 conv 的 reconcile/load/reload 改 `.then((_) => _backfillPreview(...))` 链式回填，复用幂等 `_backfillPreview`）/链式 vs 并发、失败不回填、仅活跃 conv、失效模型不变 5 条要点；§2 目标 8、§3 E 行、§8 V12、§11 server_store 行、§12.12 同步 | ✅ |
...
**结论**：LPS-14（🟡 中）已按建议并入本设计 §6.6（E 路径），与 A–D 同列为核心改动，复用现成 `_backfillPreview`、链式 `.then` 保证 reconcile 完成后回填、失败不回填与既有语义一致；LPS-15（🟢 低）列为 §10.1 follow-up。无阻塞项。二次修订 + 三次修订（§6.6 E 路径）经七轮评审 + 本轮修复复审，**可进入实现**（D 路径 + E 路径 + §12.11/§12.12 测试一并落地）。
