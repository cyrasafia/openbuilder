...
- **（LPS-18）E 路径 REST 主路径单测**：现 §12.12 测试用 discard port（reconcile 必失败）+ 手动注入消息，仅锁「`.then` 链式回填机制」，未覆盖「REST fetch→merge→backfill」端到端 happy path。补一个 mock `OpencodeClient`（返回指定 messages 列表）的测试验证 reconcile 真正合并 REST 消息后 `_lastMessage` 更新。非阻塞（机制已由现测试锁定）。
- **（LPS-20）`!loaded` 首载失败+重试成功时 `_lastMessage` 不桥接**：`load().then(_backfillPreview)` 仅首次 reconcile 尝试后回填；首次失败→`_scheduleLoadRetry` 经 timer→`_attemptLoad()` 直接调用（不经 `.then`）→重试成功时 `_messages` 已填充但 `_lastMessage` 不回填。同理 `_attemptLoad` 在 `_reconciling` 时早返→`load()` 立即 resolve→`.then` 读 pre-reconcile 状态。边角、低概率；修复需 `_attemptLoad` 成功分支回调 serverStore 或重试也走 `.then` 链。非阻塞。
