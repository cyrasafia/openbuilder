...
- **项目图标 / 名称**：服务端 `Project`（schema v2）含 `name?` 与 `icon?{url,override,color}`，经 `GET /project` 下发——desktop 连远端即取到远端项目 logo。渲染优先级：`icon.override`（用户自定义）> `icon.url`（服务端自动发现）> 客户端回退。
  ...
  - ⚠️ **SDK 注意**：发布的 v1 生成类型 `types.gen.ts` 滞后，`Project` 仅 `{id,worktree,vcsDir?,vcs?,time}`，**缺 name/icon**。desktop 用 `@opencode-ai/sdk/v2/client`（完整）。手机端须用 v2 SDK 或手动补类型，否则取不到 icon。
