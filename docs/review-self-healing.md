...
## 🔴 R1 — 前台 `_reconcile()` 的会话 reload 与列表抓取耦合（弱网下会话不更新）
...
## 🟡 R2 — `resume()` 的 `activeConv.busy` 用的是 pause 前旧 status
...
| 编号 | 问题 | 优先级 | 修复成本 |
|------|------|--------|----------|
| R1 | 前台 `_reconcile()` 会话 reload 与列表抓取耦合 | 🔴 高 | 低（挪 try 边界） |
| R2 | `resume()` 用旧 `busy` 判断 | 🟡 中 | 低（加一行 setStatus） |
| R3 | 详情页无手动刷新 | 🟡 中（UX） | 中（加控件） |
| R4 | 缺单元测试 | 🟢 低 | 中（写 mock 测试） |

R1 是弱网下「会话卡住不更新」的直接成因，且与 efb53a1 给后台场景的修复对称，建议优先修。
</content>
</invoke>